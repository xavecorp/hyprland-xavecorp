...
if [ "$(plymouth-set-default-theme)" != "xavecorp" ]; then
  sudo cp -r "$HOME/.local/share/xavecorp/default/plymouth" /usr/share/plymouth/themes/xavecorp/
  sudo plymouth-set-default-theme -R xavecorp
fi
...
if [ ! -f /etc/systemd/system/xavecorp-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/xavecorp-seamless-login.service
[Unit]
Description=Xavecorp Seamless Auto-Login
Documentation=https://github.com/basecamp/xavecorp
...
# Enable xavecorp-seamless-login.service only if not already enabled
if ! systemctl is-enabled xavecorp-seamless-login.service | grep -q enabled; then
  sudo systemctl enable xavecorp-seamless-login.service
fi
